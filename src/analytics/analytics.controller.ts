...
export class AnalyticsController {
  ...
  getWins(@Req() req: Request, @Param('gameType') gameType: IFront.GameType) {
    return this.analyticsService.getWins({ userId: req.user._id, gameType });
  }

  @UseGuards(AuthGuard('jwt'))
  @Get('get-wins-to-games/:gameType')
  getWinsToGames(@Req() req: Request, @Param('gameType') gameType: IFront.GameType) {
    return this.analyticsService.getWinsToGames({ userId: req.user._id, gameType });
  }

  @UseGuards(AuthGuard('jwt'))
  @Get('get-rating/:gameType')
  async getRating(@Req() req: Request, @Param('gameType') gameType: IFront.GameType) {
